...
# 删除相关行和函数调用
# 该命令使用sed工具对~/.bashrc文件进行就地（-i）编辑，删除满足以下任一条件的行：
# 1. 以“# 开启系统代理”开头的行
# 2. 以“# 关闭系统代理”开头的行
# 3. 以“# 关闭系统函数”开头的行
# 4. 以“# 检查clash进程是否正常启动”开头的行
# 5. 以proxy_on结尾的行
# 6. 以#开头且后面包含proxy_on的注释行
# 每个条件之间用分号分隔，表示多重匹配规则，匹配到的行会被删除（d）。
# 其中，"/^" 是正则表达式的写法，表示“以某内容开头”。例如 /^#/ 表示以#开头的行，/^# 开启系统代理/ 表示以“# 开启系统代理”开头的行。
sed -i '/^# 开启系统代理/d; /^# 关闭系统代理/d; /^# 关闭系统函数/d; /^# 检查clash进程是否正常启动/d; /proxy_on$/d; /^#.*proxy_on/d' ~/.bashrc

# 下面这行命令会处理~/.bashrc文件，删除所有连续的空行，只保留一个空行，避免出现多余的空白行。
# sed -i：表示直接修改文件内容（in-place）。
# '/^$/N;': 匹配到空行（^$），然后用N命令将下一行读入模式空间，形成两行的组合。
# '/^\n$/D'：如果这两行组合正好是两个连续的空行（即第一行和第二行都是空行），就用D命令删除模式空间的第一行，然后继续循环处理，直到没有连续空行为止。
sed -i '/^$/N;/^\n$/D' ~/.bashrc
...
if [ -f "$Config_File" ]; then
    ...
    if curl -H "User-Agent: Clash/1.0" -o /dev/null -L -k -sS --retry 5 -m 10 --connect-timeout 10 -w "%{http_code}" "$URL" | grep -E '^[23][0-9]{2}$' &>/dev/null; then
        echo "Clash订阅地址可访问！"
        
        echo -e '\n正在下载Clash配置文件...'
        # if curl -L -k -sS --retry 5 -m 30 -o "$Config_File" "$URL"; then
        if curl -H "User-Agent: Clash/1.0" -L -k -sS --retry 5 -m 30 -o "$Config_File" "$URL"; then
            echo "配置文件下载成功！"
        else
...
# 获取Clash端口（如果yq可用）
if [ -x "$YQ_BINARY" ]; then
    # 尝试获取 mixed-port，如果没有则尝试 port，最后使用默认值
    CLASH_PORT=$($YQ_BINARY eval '.mixed-port // .port // 7890' $Config_File 2>/dev/null || echo "7890")
else
